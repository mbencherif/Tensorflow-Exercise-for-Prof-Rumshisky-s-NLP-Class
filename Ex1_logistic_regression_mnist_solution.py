...
input_per_batch = tf.placeholder(tf.float32, [batch_size, 784], name='input_per_batch')
label_per_batch = tf.placeholder(tf.float32, [batch_size, 10], name='label_per_batch')
...
logits_batch = tf.matmul(input_per_batch, w) + b
...
entropy = tf.nn.softmax_cross_entropy_with_logits(logits = logits_batch, labels = label_per_batch)
...
with tf.Session() as sess:
	...
	for i in range(n_epochs): # train the model n_epochs times
		...
		for _ in range(n_batches):
			...
			_, loss_batch = sess.run([optimizer, loss], feed_dict={input_per_batch:X_batch, label_per_batch: Y_batch})
